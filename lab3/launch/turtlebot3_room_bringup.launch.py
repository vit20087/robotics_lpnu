"""Launch TurtleBot3 in room world + path publisher + RViz."""
import os
from launch import LaunchDescription
from launch.actions import SetEnvironmentVariable, IncludeLaunchDescription, ExecuteProcess, DeclareLaunchArgument, OpaqueFunction, AppendEnvironmentVariable
...
def launch_setup(context):
    ...
    rviz_config = os.path.join(lab3_share, 'rviz', 'trajectory.rviz')
    ...
    odom_path = ExecuteProcess(
        cmd=['ros2', 'run', 'lab3', 'odom_path_publisher', '--ros-args', '-p', 'odom_topic:=/odom'],
        output='screen'
    )

    actions = [gzserver, gzclient, spawn_turtlebot, robot_state_publisher, odom_path]
    if use_rviz:
